...
src_prepare() {
	...
	eapply     "${FILESDIR}"/exim-4.99-as-needed-ldflags.patch # 352265, 391279
	...
	# exim-4.74-radius-db-ENV-clash.patch dropped: call_radius.c no longer
	# exists; radius is now in src/miscmods/radius.c which handles the
	# ENV/DB clash itself via a #define rename.
	# exim-4.98-tidydb-crash.patch dropped: fixed upstream in 4.99.3;
	# the per-backend hintsdb headers now include the COMPILE_UTILITY guard.
